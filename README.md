...
- **分割(無劣化)** — 長い動画を「約25MB(Claude用)」または15 / 30 / 60秒ごとに分割できます。**再エンコードしないので画質は元のまま**、処理も短時間で終わります(MP4 / MOV のみ)
...
**分割**: [mp4box.js](https://github.com/gpac/mp4box.js)(`vendor/mp4box.min.js`)で MP4 / MOV を解析してサンプル単位で取り出し、サンプルテーブル(stbl)を持つ**通常の MP4** を組み直します。コーデック設定(stsd)と回転情報(tkhd の matrix)は元ファイルから引き継ぐため、縦向き動画も正しく表示されます。再エンコードしないので画質の劣化がなく、分割位置は必ずキーフレームに合わせます(そのため指定した秒数・大きさちょうどにはなりません)。

サイズ指定のときは、目標を超えない範囲で最も遠いキーフレームを選ぶため、各ファイルは目標以下に収まります(キーフレーム間隔が目標より長い場合を除く)。
